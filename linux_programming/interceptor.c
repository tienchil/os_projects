...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// Special case: current->pid = 0
	// If a the pid is 0, it should not appear in any pid list

	if ((current->pid != 0) && (table[reg.ax].monitored == 2)) {

		// Check if the syscall is not monitored for this pid
		int not_monitored = check_pid_monitored(reg.ax, current->pid);

		if (not_monitored == 0) {

			// PID is not in the blacklist
	...
	} else if ((current->pid != 0) && (table[reg.ax].monitored == 1)) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	/* Check All Error Conditions before proceeding. */
	...
	if (syscall < 0) {
		...
		printk(KERN_DEBUG "System Call number < 0\n");
	...
	if (syscall > NR_syscalls) {
		...
		printk(KERN_DEBUG "System Call number exceeds limit\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	}
	...
	if (syscall == MY_CUSTOM_SYSCALL) {
		...
		printk(KERN_DEBUG "System Call number is the interceptor\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	}
	...
	if (pid < 0) {
		...
		printk(KERN_DEBUG "Invalid PID: Negative number\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	...
	if ((pid != 0) && (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) {
		...
		printk(KERN_DEBUG "PID does not belong to a valid process\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	...
	// Check the called has the right permissions.
	if ((cmd == REQUEST_SYSCALL_INTERCEPT) || \
	    (cmd == REQUEST_SYSCALL_RELEASE))  {

		if (current_uid() != 0) {
			printk(KERN_DEBUG "You're not the root\n");
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EPERM;
		}
	...
	if ((cmd == REQUEST_START_MONITORING) || \
		...
		if (current_uid() != 0) {
			...
			if ((pid != 0) && \
				(check_pid_from_list(current->pid, pid) != 0)) {
				
				printk(KERN_DEBUG \
				   "PID is not owned by the calling process\n");
				spin_unlock(&pidlist_lock);
				spin_unlock(&calltable_lock);
				return -EPERM;
				...
				printk(KERN_DEBUG \
				   "PID is 0 but calling process is not root\n");
				spin_unlock(&pidlist_lock);
				spin_unlock(&calltable_lock);
				return -EPERM;
			}
	...
	if ((cmd == REQUEST_SYSCALL_RELEASE) && \
	   (table[syscall].intercepted == 0)) {
		...
		printk(KERN_DEBUG "Cannot release un-intercepted system call\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	}
	...
	if (cmd == REQUEST_STOP_MONITORING) {
		...
		// The pid is not being monitored
		if ((pid != 0) && (check_pid_monitored(syscall, pid) == 0)) {

			printk(KERN_DEBUG \
			   "Cannot stop monitoring a pid that is "
			   "not being monitored\n");
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EINVAL;
			...
			printk(KERN_DEBUG \
			   "Cannot stop monitoring un-intercepted system call\n");
			spin_unlock(&pidlist_lock);
			spin_unlock(&calltable_lock);
			return -EINVAL;
		}
	...
	if ((cmd == REQUEST_START_MONITORING) && \
		...
		printk(KERN_DEBUG "Cannot start monitoring a pid" \
				  "for an un-intercepted system call\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EINVAL;
	}
	...
	if ((cmd == REQUEST_SYSCALL_INTERCEPT) && \
		...
		printk(KERN_DEBUG "Cannot intercept an intercepted system call\n");
	...
	if ((pid != 0) && (cmd == REQUEST_START_MONITORING) && \
	    (check_pid_monitored(syscall, pid) == 1)) {

		printk(KERN_DEBUG "Cannot start monitoring" 
				  "a pid already being monitored\n");
		spin_unlock(&pidlist_lock);
		spin_unlock(&calltable_lock);
		return -EBUSY;
	...
	/* End of Checking Error Condition.*/

	spin_unlock(&pidlist_lock);
	spin_unlock(&calltable_lock);

	/* Implementation of my_syscall functionality.*/

	// Intercept a systam call
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		// Store the intercepted one into syscall table
		...
		// Release a system call

		spin_lock(&calltable_lock);

		// Restore the original syscall back to syscall table
		set_addr_rw((unsigned long) sys_call_table);
		...
		// Change the corresponding data in table
		spin_lock(&pidlist_lock);
		...
		// Add this pid to those syscalls that monitor all pids
		// as a blacklist.
			...
			// Check if this syscall monitors all pids
				add_pid_sysc(pid, syscall);
...
{        
	...
	// Restore MY_CUSTOM_SYSCALL and exit_group to original
